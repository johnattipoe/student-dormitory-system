...
function getSettingsDraftKey(form: HTMLFormElement): string {
  return `sds.settings-draft:${window.location.pathname}:${form.id || 'default'}`;
}

function getDraftControls(form: HTMLFormElement): HTMLInputElement[] {
...
function readSettingsDraft(form: HTMLFormElement): SettingsDraftValue[] {
...
function writeSettingsDraft(form: HTMLFormElement, draft: SettingsDraftValue[]): void {
...
function initialiseSettingsDraft(form: HTMLFormElement): void {
  const settingsDraftKey = getSettingsDraftKey(form);
  const storedDraft = window.localStorage.getItem(settingsDraftKey);
  if (storedDraft) {
    try {
      const draft = JSON.parse(storedDraft) as SettingsDraftValue[];
      if (Array.isArray(draft)) writeSettingsDraft(form, draft);
    } catch {
...
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll<HTMLFormElement>('[data-ts-settings-form]').forEach(initialiseSettingsDraft);
});
