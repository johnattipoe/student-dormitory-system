...
abstract class QueueableJob implements JobContract
{
    ...
    public function __construct(string $jobId, int $attempt = 1)
    {
        $this->jobId = $jobId;
    ...
    protected function completed(array $data = []): array
    {
        return ['jobId' => $this->jobId, 'job' => $this->name(), 'attempt' => $this->attempt, 'completedAt' => date(DATE_ATOM)] + $data;
    }
}

final class SendNotificationJob extends QueueableJob
{
    public function __construct(string $jobId, private string $userId, private string $title, private string $message, private string $type = 'info', int $attempt = 1) { parent::__construct($jobId, $attempt); }
...
{
    public function __construct(string $jobId, private string $reportType, private array $filters = [], int $attempt = 1) { parent::__construct($jobId, $attempt); }
...
{
    public function __construct(string $jobId, private string $attendanceId, int $attempt = 1) { parent::__construct($jobId, $attempt); }
