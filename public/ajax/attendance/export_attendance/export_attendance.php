...
$houseId = sanitize($_GET['houseId'] ?? (current_role() === ROLE_SENIOR_HOUSEPARENT ? null : (current_user()['houseId'] ?? null)));
$allHouses = (int) ($_GET['all'] ?? 0);

// Houseparent can view all houses if 'all=1'
if (current_role() === ROLE_SENIOR_HOUSEPARENT && !$allHouses) {
    // Show filtered by house
} elseif (current_role() === ROLE_SENIOR_HOUSEPARENT && $allHouses) {
