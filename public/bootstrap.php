...
// Advertise the favicon before any page component can emit markup.
header('Link: </favicon.ico?v=mawuli-school-3>; rel="icon"; type="image/x-icon"', false);
...
if (!str_contains($requestUri, '/ajax/') && !str_contains($requestUri, '/reports/') && !$acceptsJson && !$isXmlHttpRequest && !$isSettingsExport && !$isStudentTemplateDownload) {
    ...
    $legacyLoadingMarkup = ob_get_clean();
}
