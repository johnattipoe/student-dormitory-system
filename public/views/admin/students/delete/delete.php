...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    redirect(url('views/admin/students/index/index.php?deleted=1'));
}
    ...
    <div class="content-wrapper">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
            <div>
                <h4 class="mb-1 fw-bold text-danger">
                    <i class="bi bi-person-x-fill me-2"></i>Delete Student Record
                </h4>
                <p class="text-muted mb-0">Permanently remove student from the dormitory database</p>
            </div>
            <div>
        ...
        <div class="card stat-card shadow-sm border-0 border-top border-4 border-danger" style="max-width: 600px;">
            <div class="card-body p-4 text-center">
                ...
                <h5 class="fw-bold text-dark mb-2">Delete <?= e($fullName) ?>?</h5>
                <p class="text-muted mb-3">
                    Admission Number: <strong class="font-monospace text-dark"><?= e($student['admissionNo'] ?? '—') ?></strong>
                </p>
                <div class="alert alert-danger text-start py-2 small mb-4">
                    <i class="bi bi-exclamation-triangle-fill me-2"></i><strong>Warning:</strong> This action cannot be reversed. All attendance, room allocation, and incident history linked to this student record will be permanently deleted.
                </div>
                <form method="POST" action="<?= url('views/admin/students/delete/delete.php?id=' . urlencode($id)) ?>" class="d-flex justify-content-center gap-2">
                    <input type="hidden" name="id" value="<?= e($id) ?>">
                    <button type="submit" class="btn btn-danger">
                        <i class="bi bi-trash me-1"></i> Permanently Delete
                    </button>
                    <a class="btn btn-outline-secondary" href="<?= url('views/admin/students/index/index.php') ?>">Cancel</a>
                </form>
            </div>
        </div>
