...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors && $action !== 'export') {
        try {
            ...
            redirect(url('views/admin/settings/advanced/advanced.php'));
        } catch (Throwable $e) {
...
$navItems = [
    ...
    ['icon' => 'bi-sliders', 'label' => 'Advanced Settings', 'href' => url('views/admin/settings/advanced/advanced.php'), 'active' => true],
];
    ...
    <div class="content-wrapper admin-settings-page advanced-settings-page">
        ...
        <form id="advanced-settings-form" method="POST" enctype="multipart/form-data">
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-6"><div class="card stat-card p-3 h-100"><h6>Backup & Audit</h6><div class="mb-3"><label class="form-label">Import settings JSON</label><input type="file" name="settings_file" accept="application/json,.json" class="form-control"><small class="text-muted">Imported values are validated before saving.</small></div><div class="d-flex gap-2"><button class="btn btn-outline-primary btn-sm" type="submit" formaction="<?= url('views/admin/settings/advanced/advanced.php') ?>" name="action" value="import"><i class="bi bi-upload me-1"></i>Import and save</button><button class="btn btn-outline-danger btn-sm" type="submit" name="action" value="reset" formnovalidate onclick="return confirm('Reset all advanced settings to defaults?')"><i class="bi bi-arrow-counterclockwise me-1"></i>Reset defaults</button></div><hr><h6 class="small">Recent changes</h6><?php $history = $settingsDocument['history'] ?? []; ?><?php if (!$history): ?><p class="text-muted small mb-0">No changes recorded yet.</p><?php else: ?><ul class="list-unstyled small mb-0"><?php foreach (array_slice($history, 0, 5) as $entry): ?><li class="mb-2"><strong><?= e($entry['action'] ?? 'save') ?></strong> by <?= e($entry['updatedBy'] ?? 'admin') ?><br><span class="text-muted"><?= e($entry['updatedAt'] ?? '') ?></span></li><?php endforeach; ?></ul><?php endif; ?></div></div>
            </div>
        </form>
    </div>
</div>
