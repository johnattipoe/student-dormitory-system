                            ...
                            <?php foreach ($attendanceHistory as $entry): ?>
                                ...
                                <tr>
                                    ...
                                    <td class="text-end text-nowrap">
                                        <button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#houseHistoryViewModal-<?= e((string) ($entry['id'] ?? '')) ?>"><i class="bi bi-eye me-1"></i>View</button>
                                        <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#houseHistoryEditModal-<?= e((string) ($entry['id'] ?? '')) ?>"><i class="bi bi-pencil me-1"></i>Edit</button>
                                        <button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#houseHistoryDeleteModal-<?= e((string) ($entry['id'] ?? '')) ?>"><i class="bi bi-trash"></i></button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
...
<?php foreach ($attendanceHistory as $entry): ?>
    <?php $modalHistoryId = (string) ($entry['id'] ?? ''); $modalHistoryStudent = $studentMap[(string) ($entry['studentId'] ?? '')] ?? []; $modalHistoryStatus = (string) ($entry['status'] ?? 'present'); ?>
    <div class="modal fade" id="houseHistoryViewModal-<?= e($modalHistoryId) ?>" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">Attendance Details</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div><div class="modal-body"><dl class="row mb-0"><dt class="col-sm-5">Student</dt><dd class="col-sm-7"><?= e(trim(($modalHistoryStudent['firstName'] ?? '') . ' ' . ($modalHistoryStudent['lastName'] ?? '')) ?: ($entry['studentId'] ?? '—')) ?></dd><dt class="col-sm-5">Date</dt><dd class="col-sm-7"><?= e($entry['date'] ?? '—') ?></dd><dt class="col-sm-5">Status</dt><dd class="col-sm-7"><?= e(ucfirst($modalHistoryStatus)) ?></dd><dt class="col-sm-5">Marked By</dt><dd class="col-sm-7"><?= e($getMarkedByName($entry)) ?></dd></dl></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button></div></div></div></div>
    <div class="modal fade" id="houseHistoryEditModal-<?= e($modalHistoryId) ?>" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><form method="POST" action="<?= url('views/house-master/attendance/edit/edit.php?id=' . urlencode($modalHistoryId)) ?>"><div class="modal-header"><h5 class="modal-title">Edit Attendance</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div><div class="modal-body"><input type="hidden" name="id" value="<?= e($modalHistoryId) ?>"><label class="form-label">Date</label><input type="date" name="date" class="form-control mb-3" value="<?= e($entry['date'] ?? date('Y-m-d')) ?>"><label class="form-label">Status</label><select name="status" class="form-select"><option value="present" <?= $modalHistoryStatus === 'present' ? 'selected' : '' ?>>Present</option><option value="absent" <?= $modalHistoryStatus === 'absent' ? 'selected' : '' ?>>Absent</option><option value="late" <?= $modalHistoryStatus === 'late' ? 'selected' : '' ?>>Late</option><option value="excused" <?= $modalHistoryStatus === 'excused' ? 'selected' : '' ?>>Excused</option></select></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button><button type="submit" class="btn btn-primary">Save changes</button></div></form></div></div></div>
    <div class="modal fade" id="houseHistoryDeleteModal-<?= e($modalHistoryId) ?>" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><div class="modal-header border-0"><h5 class="modal-title text-danger">Delete Attendance</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div><div class="modal-body"><p class="mb-0">Delete this attendance record?</p></div><div class="modal-footer border-0"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button><form method="POST" action="<?= url('views/house-master/attendance/delete/delete.php?id=' . urlencode($modalHistoryId)) ?>"><input type="hidden" name="id" value="<?= e($modalHistoryId) ?>"><button type="submit" class="btn btn-danger">Delete</button></form></div></div></div></div>
<?php endforeach; ?>
