...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$isAdmin) {
        ...
        if (!$student || !$room || (string) ($student['houseId'] ?? '') !== $houseId || (string) ($room['houseId'] ?? '') !== $houseId) {
            ...
            redirect(url('views/house-master/rooms/allocation.php'));
        }
    }
    if ($action === 'remove') {
    ...
    redirect(url('views/' . ($isAdmin ? 'admin' : 'house-master') . '/rooms/allocation.php'));
}
