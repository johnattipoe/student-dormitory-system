                    ...
                    <?php if (!empty($visitorRequests)): ?>
                        <?php foreach ($visitorRequests as $request): ?>
                            <tr>
                                ...
                                <td><span class="badge bg-<?= ($request['status'] ?? '') === 'approved' ? 'success' : 'secondary' ?>"><?= e($request['status'] ?? 'pending') ?></span></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="4" class="text-center text-muted">No visitor requests yet.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
