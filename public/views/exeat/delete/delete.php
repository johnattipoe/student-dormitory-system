...
// Admins can delete any record they have already loaded; other roles remain scoped.
$visibleIds = $role === ROLE_ADMIN
    ? [$id => true]
    : array_fill_keys(array_map(static fn(array $r): string => (string) ($r['id'] ?? ''), $service->visibleForRole($role, $userId, $houseId, $studentId)), true);
...
$student = $role !== ROLE_ADMIN && $recStudentId !== '' ? StudentService::find($recStudentId) : null;
