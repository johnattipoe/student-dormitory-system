...
?>
<style>
.exeat-create-form .nav-pills .nav-link {
    border-radius: 10px;
    font-size: 0.9rem;
    padding: 0.7rem 1rem;
}
.exeat-create-form .nav-pills .nav-link.active {
    box-shadow: 0 2px 8px rgba(13, 110, 253, 0.18);
}
.exeat-create-form .form-control,
.exeat-create-form .form-select,
.exeat-create-form .input-group-text {
    min-height: 42px;
}
.exeat-create-form .card-body {
    padding: 1.25rem 1.5rem;
}
.exeat-create-form .alert {
    margin-bottom: 1rem;
}
.exeat-create-form .btn {
    min-height: 38px;
}
</style>
<div class="main-content">
    ...
    <div class="content-wrapper">
        ...
        <div class="card stat-card shadow-sm border-0 exeat-create-form" style="max-width: 820px;">
            ...
            <div class="card-body p-4">
                <form method="POST" action="<?= url('views/exeat/create/create.php') ?>" class="exeat-create-form-body">
                    <?= csrf_field() ?>
                    <input type="hidden" name="exeatType" id="selectedExeatType" value="<?= e($selectedType) ?>">

                    <?php if ($canCreateForStudent): ?>
                        <div id="studentSelectorSection" class="mb-4">
                            <label class="form-label fw-semibold">Select Student <span class="text-danger">*</span></label>
                            <div class="student-search-wrap">
                                <i class="bi bi-search search-icon"></i>
                                <input type="text" id="studentSearchInput" class="form-control student-search-input" list="studentOptions" placeholder="Search student by name or admission number..." aria-label="Search student" autocomplete="off" required <?= empty($houseStudents) ? 'disabled' : '' ?>>
                                <datalist id="studentOptions">
                                    <?php foreach ($houseStudents as $student): ?>
                                        ...
                                        <?php if ($optionStudentId !== ''): ?>
                                            <option value="<?= e(($studentName !== '' ? $studentName : 'Unnamed student') . ' (' . ($student['admissionNo'] ?? $student['studentId'] ?? $optionStudentId) . ')' . ($sHouseName ? ' - ' . $sHouseName : '')) ?>" data-student-id="<?= e($optionStudentId) ?>"></option>
                                        <?php endif; ?>
                                    <?php endforeach; ?>
                                </datalist>
                    ...
                    <div class="row g-3 mt-1">
                        ...
                        <div id="guardianPhoneSection" class="col-12 <?= $selectedType === 'internal' ? 'd-none' : '' ?>">
...
function setType(type) {
    ...
    if (type === 'internal') {
        ...
        if (studentSelectorSection) studentSelectorSection.classList.remove('d-none');
        if (guardianPhoneSection) guardianPhoneSection.classList.add('d-none');
    } else {
        internalFields.classList.add('d-none');
        externalFields.classList.remove('d-none');
        if (studentSelectorSection) studentSelectorSection.classList.remove('d-none');
        if (guardianPhoneSection) guardianPhoneSection.classList.remove('d-none');
    }
}
...
function updateStudentDetails() {
    ...
    if (!studentIdInput || !guardianPhoneInput) return;
    ...
    if (studentContactBox) {
        if (student && studentId && document.getElementById('selectedExeatType').value === 'external') {
...
function setupStudentSearch() {
    ...
    if (!searchInput || !studentIdInput) return;

    searchInput.addEventListener('input', function () {
        const selectedOption = Array.from(studentOptions).find(option => option.value === this.value);
        studentIdInput.value = selectedOption ? selectedOption.dataset.studentId : '';
        updateStudentDetails();
    });
}
